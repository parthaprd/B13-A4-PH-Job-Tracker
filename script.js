...
function setTab(tabName) {
  ...
  tabButtons.forEach(button => {
    button.className = "tab-btn px-4 py-2 rounded-md text-sm font-medium border bg-white text-slate-700 text-center";
  });


  const activeButton = document.getElementById(`tab-${tabName.toLowerCase()}`);
  activeButton.className = "tab-btn px-4 py-2 rounded-md text-sm font-medium bg-blue-500 text-white text-center";
    ...
    jobList.innerHTML += `
      <div class="bg-white border rounded-xl p-6 relative">
        ...
        <div class="flex gap-3 mt-5">
          <button onclick="updateStatus(${job.id}, 'INTERVIEW')" class="px-4 py-2 rounded border ${job.status === 'INTERVIEW' ? 'bg-green-500 text-white' : 'text-green-600 border-green-500 hover:bg-green-50'
      }">INTERVIEW</button>

          <button onclick="updateStatus(${job.id}, 'REJECTED')" class="px-4 py-2 rounded border ${job.status === 'REJECTED' ? 'bg-red-500 text-white' : 'text-red-600 border-red-500 hover:bg-red-50'
      }">REJECTED</button>
        </div>
      </div>
    `;
...
function render() {
  ...
  setTab('ALL'); 
>>>>>>> f1455980cc985637b9507f2ed032acba79dfad69
